	...
	*local death_type amen
		...
		foreach age in 55_64 65_74 {
			*local age 55_64
			...
			////////////////////////////////////////////////////////////////////////
			// With controls
			reghdfe ln_`death_type'_`age'   i.treatment i.post i.expansion Mcare_Adv_Penet Mcare_Disab IncPC IncMedHH PovertyPrcnt UnempRate diabetes_prev_rate_age_adj inactivity_percent_age_adj obesity_percent smoking_all physicians_pc  [aw = attpop_`age'] , absorb(state year) cluster(state)
			...
			// Store the model
			est save "$table_results_path/state_ln_`death_type'_`age'_c", replace
	...
	*local death_type amen
		...
		foreach age in 55_64 65_74 {
			*local age 55_64
			...
			////////////////////////////////////////////////////////////////////////
			// With controls
			reghdfe ln_amen_`dem_group'_`age'   i.treatment i.post i.expansion Mcare_Adv_Penet Mcare_Disab IncPC IncMedHH PovertyPrcnt UnempRate diabetes_prev_rate_age_adj inactivity_percent_age_adj obesity_percent smoking_all physicians_pc  [aw = attpop_`dem_group'_`age' ] , absorb(state year) cluster(state)
			...
			// Store the model
			est save "$table_results_path/state_ln_amen_`dem_group'_`age'_c", replace
	...
	reshape long ///
		...
		ln_namen_@ ln_any_@ attpop_@ pop_@, i(state year) j(age) string
	rename *_ *
	
	save $temp_path/state_temp_mort_data.dta, replace
	clear
	// By cause-of-death
	foreach death_type in amen namen any resp can card hiv dia {

		use $temp_path/state_temp_mort_data.dta, clear
		...
		reghdfe ln_`death_type' ///
			...
			i.ddd_treat ///
			`list_of_controls' ///
			[aw = attpop] , absorb(state year) cluster(state)
		...
		// Store the model
		est save "$table_results_path/state_ln_`death_type'_ddd_c", replace
	...
	foreach dem_group in male fem white_non_hisp black_non_hisp hisp {
		...
		// Store the model
		est save "$table_results_path/state_ln_amen_`dem_group'_ddd_c", replace
	...
	// Export results
		...
		esttab ///
			...
			state_ln_amen_ddd_c ///
			using "$table_results_path/state_appendix_regression_results_`file_name_suffix'.tex" ///
			,star(* 0.10 ** 0.05 *** .01) ///
			stats( ///
				dd_b_str dd_se_str ///
				, fmt(0 0 ) ///
	...
	foreach death_type in  namen any resp can card hiv   {
		if "`death_type'" == "namen" {
	...
	foreach dem_group in   male fem white black hisp  {
		if "`dem_group'" == "male" {
		...
		esttab ///
			state_ln_dia_55_64_c ///
			 state_ln_dia_65_74_c ///
			 state_ln_dia_ddd_c ///
			using "$table_results_path/state_appendix_regression_results_`file_name_suffix'.tex" ///
			,star(* 0.10 ** 0.05 *** .01) ///
			stats( ///
				...
				label("Hispanic, amenable mortality" "\vspace{.25cm}~" ///
					" \addlinespace \hline Weights" "State fixed-effects" "Year fixed-effects" "Covariates")) ///
				drop(*) ///
				se ///
				b(%9.2f) ///
				booktabs ///
...
// Clean up 
erase "$temp_path/state_temp_mort_data.dta"
