...
capture program drop regression_results

// *----------
program define regression_results
syntax, ///
	weight_var(string) /// 
	weight_label(string) /// 
	y_var_prefix(string) ///   
	...
	*local death_type amen
		...
		foreach age in 55_64 65_74 {
			*local age 55_64
			...
			////////////////////////////////////////////////////////////////////////		
			// Without controls
			reghdfe `y_var_prefix'_`death_type'_`age'  i.treatment i.post i.expansion  [aw = `weight_var'_`age'] , absorb(unique_county_id year) cluster(`cluster_var')
			...
			// Store the model
			est save "$table_results_path/`y_var_prefix'_`death_type'_`age'_nc", replace
			
			////////////////////////////////////////////////////////////////////////
			// With controls
			reghdfe `y_var_prefix'_`death_type'_`age'   i.treatment i.post i.expansion `list_of_controls'  [aw = `weight_var'_`age'] , absorb(unique_county_id year) cluster(`cluster_var')
			...
			// Store the model
			est save "$table_results_path/`y_var_prefix'_`death_type'_`age'_c", replace
	...
	reshape long `y_var_prefix'_amen_@ `y_var_prefix'_namen_@ `y_var_prefix'_any_@ attpop_@ pop_@, i(unique_county_id year) j(age) string
	rename *_ *
	...
	foreach death_type in amen namen any {
		...
		// Store the model
		est save "$table_results_path/`y_var_prefix'_`death_type'_ddd_nc", replace
		...
		// Store the model
		est save "$table_results_path/`y_var_prefix'_`death_type'_ddd_c", replace
...
**MAKE SURE THIS ONE IS LAST SO THAT THE ESTIMATES ARE NOT OVERWRITTEN
