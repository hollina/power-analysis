...
local weight_label "`weight_label'"  // weight label (pop attpop)
local y_var_prefix dr  
local cluster_var state // state or county
local file_name_suffix dr_amenable_attpop // end of file name, before png
...
*local death_type amen
	...
	*local age 55_64
		...
		////////////////////////////////////////////////////////////////////////		
		// Without controls
		reghdfe dr_`death_type'_`age'  i.treatment i.post i.expansion  [aw = `weight_var'_`age'] , absorb(unique_county_id year) cluster(`cluster_var')
		...
		// Store the model
		est save "$table_results_path/dr_`death_type'_`age'_nc", replace
		
		////////////////////////////////////////////////////////////////////////
		// With controls
		reghdfe dr_`death_type'_`age'  i.treatment i.post i.expansion `list_of_controls' [aw = `weight_var'_`age'] , absorb(unique_county_id year) cluster(`cluster_var')
		...
		// Store the model
		est save "$table_results_path/dr_`death_type'_`age'_c", replace
...
foreach death_type in amen namen any {
	...
	// Store the model
	est save "$table_results_path/dr_`death_type'_ddd_nc", replace
	...
	// Store the model
	est save "$table_results_path/dr_`death_type'_ddd_c", replace
...
foreach death_type in amen namen any  {	
	...
	foreach age in 55_64 65_74 ddd {
		est use "$table_results_path/dr_`death_type'_`age'_nc.ster"
		est sto dr_`death_type'_`age'_nc
		est use "$table_results_path/dr_`death_type'_`age'_c.ster"
		est sto dr_`death_type'_`age'_c
	}
}
...
// Export results
	...
	esttab ///
		dr_amen_55_64_nc dr_amen_55_64_c ///
		dr_amen_65_74_nc dr_amen_65_74_c ///
		dr_amen_ddd_nc dr_amen_ddd_c ///
		using "$table_results_path/regression_results_`file_name_suffix'.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
		stats( ///
			dd_b_str dd_se_str ///
	...
	esttab ///
		dr_namen_55_64_nc dr_namen_55_64_c ///
		dr_namen_65_74_nc dr_namen_65_74_c ///
		dr_namen_ddd_nc dr_namen_ddd_c ///
		using "$table_results_path/regression_results_`file_name_suffix'.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
		stats( ///
			dd_b_str dd_se_str ///
	...
	esttab ///
		dr_any_55_64_nc dr_any_55_64_c ///
		dr_any_65_74_nc dr_any_65_74_c ///
		dr_any_ddd_nc dr_any_ddd_c ///
		using "$table_results_path/regression_results_`file_name_suffix'.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
		stats( ///
			dd_b_str dd_se_str ///
