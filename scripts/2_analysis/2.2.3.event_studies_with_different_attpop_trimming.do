...
foreach top_trim in 900 925 975 1000 {
	...
	// Make sure we are using the correct ATTxPop weights
	use "$restricted_data_analysis/attpop_trim_`top_trim'.dta", clear
	merge 1:1 state county year using "$public_data_analysis/county_controls.dta" 
	keep if _merge == 3
	drop _merge 
	merge 1:1 state county year using  "$restricted_data_analysis/death_rates_1999_2017.dta"
	keep if _merge == 3
	drop _merge 
