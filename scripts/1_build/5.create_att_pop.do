...
foreach top_trim in 900 925 950 975 1000 {
	...
	*local bottom_trim = 100 - `top_trim'

	use "$restricted_data_analysis/death_rates_1999_2017.dta", clear

	// Merge in control data at the county-level
	merge 1:1 state county year using "$public_data_analysis/county_controls.dta"
	keep if _merge == 3
	drop _merge 
	...
	* Account for mid-year expansions
	...
	gcollapse (mean) ///
		...
		smoking_all percent_everyday_smoker percent_someday_smoker percent_former_smoker ///
		physicians_pc expansion_4 pop* , by(state county)
	...
	*/
	...
	keep state county att 

	merge 1:m state county using "$restricted_data_analysis/death_rates_1999_2017.dta"
	drop _merge
	...
	*gen attpop_65_74 = att*pop_65_74
	...
	// Save 
	*keep state county year attpop_55_64 attpop_65_74
	compress
	save "$restricted_data_analysis/attpop_trim_`top_trim'.dta", replace
}

// Open the windsorized @ 95th percentile data
use "$restricted_data_analysis/attpop_trim_950.dta", clear
merge 1:1 state county year using "$public_data_analysis/county_controls.dta" 
keep if _merge == 3
drop _merge 
compress
save "$restricted_data_analysis/county_controls_with_att_pop.dta" , replace
