...
capture program drop st_power_analysis_ddd

// *----------
program define st_power_analysis_ddd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly {	
	...
	// Set demographic group (other than age)
	local dem_type  le
	...
	*/
	...
	local file_name_suffix st_ddd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
	...
	/*
	local att_var_list ///
		pop_male pop_white_non_hisp  pop_55_64 /// pop_65_74 pop_black_non_hisp IncPC
		 IncMedHH PovertyPrcnt UnempRate Mcare_Adv_Penet ///
		diabetes_prev_rate_age_adj inactivity_percent_age_adj ///
		pct_unins_65 physicians_pc
	*/	
	local att_var_list ///
	...
	if "`dem_type'" == "le" {
	local att_var_list ///
		pop_male pop_white_non_hisp pop_45_64 IncMedHH ///
		PovertyPrcnt UnempRate Mcare_Adv_Penet pct_unins_65
	}
	...
	** Create Event Time Variable
	...
	if "`dem_type'" == "none" {
		...
		keep ///
			state year  ///
			`death_type'_55_64 `death_type'_65_74  pop_* ///
			`list_of_controls' ///
		...
		reshape long `death_type'_@    pop__@, i(state year) j(age) string
	...
	if "`dem_type'" != "none" {
		if "`dem_type'" != "le" {
			...
			keep ///
				state year ///
				`death_type'_`dem_type'_55_64 `death_type'_`dem_type'_65_74  pop_* ///
				`list_of_controls' ///
				 real_expansion real_post ///
				 `att_var_list'
			...
			reshape long `death_type'_`dem_type'_@  pop__@, i(state year) j(age) string
	...
	if "`dem_type'" == "le" {
		...
		keep ///
			state year  ///
			`death_type'_`dem_type'_45_64 `death_type'_`dem_type'_eld  pop_* ///
			`list_of_controls' ///
			 real_expansion real_post ///
			 `att_var_list'
		...
		reshape long `death_type'_@  pop__@ ///
			///
			, i(state year) j(age) string
		rename *__ *
		rename *_ *
