...
capture program drop st_simsynth_power_analysis_ddd

// *----------
program define st_simsynth_power_analysis_ddd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly{
	...
	*/
	...
	local file_name_suffix st_simp_synth_ddd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
	...
	** Create Event Time Variable
	...
	if "`dem_type'" == "none" {
		...
		reshape long `death_type'_@ , i(state year) j(age) string
		rename *_ *
	...
	if "`dem_type'" != "none" {
		...
		reshape long `death_type'_`dem_type'_@ , i(state year) j(age) string
		rename *_ *
...
quietly {
		...
		*local first_year 2004
		...
		bysort state age: egen new_rate_normal = max(rate_normal)
		
		sort  state age year 

		capture drop order
		bysort state age: gen order = _n
		...
		*local last_year 2010
		...
		sort state  age year 
		order state age year order rate_normal new_rate_normal	
