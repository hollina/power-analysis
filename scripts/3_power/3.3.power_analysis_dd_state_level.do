...
capture program drop st_power_analysis_dd

// *----------
program define st_power_analysis_dd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly {
	...
	*/
	...
	local file_name_suffix st_dd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
		...
		*local x = 0 	
			...
			if "`prefix'" == "ln"{
				...
				qui reghdfe ln_dr_`counter' ///
					...
					i.post i.expansion ///
					`list_of_controls'  ///
					[aweight = `weight_var'_`dem_type'] ,  absorb(state year)  vce(cluster `cluster_var') 
