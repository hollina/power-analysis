...
capture program drop simp_synth_power_analysis_dd

// *----------
program define simp_synth_power_analysis_dd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly {
	...
	*/
	...
	local file_name_suffix simp_synth_dd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
	...
	*Need to do this since counties are trending. If we only pull from pre-period mean, then we will have too high of a death rate to be actually comparable.
	capture drop yhat
	capture drop rate
	capture drop fe_year_slope fe_county fe_year
