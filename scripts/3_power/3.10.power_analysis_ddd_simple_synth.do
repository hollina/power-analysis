...
capture program drop simp_synth_power_analysis_ddd

// *----------
program define simp_synth_power_analysis_ddd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly{
	...
	*/
	...
	local file_name_suffix simp_synth_ddd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
	...
	** Create Event Time Variable
	...
	if "`dem_type'" == "none" {
		...
		keep ///
			...
			*_55_64 *_65_74 /// 
			`list_of_controls' ///
			 real_expansion real_post attpop_* pop_* ///
			 `att_var_list'

		reshape long `death_type'_@ , i(unique_county_id year) j(age) string
		rename *_ *
	...
	if "`dem_type'" != "none" {
		...
		reshape long `death_type'_`dem_type'_@ , i(unique_county_id year) j(age) string
		rename *_ *
	...
	gen real_age_treat = real_treated_age*real_expansion
		...
		replace att = 1 if real_expansion == 1
	...
	// Calculate the best fit line for each county excluding the post-treatment period
	*Need to do this since counties are trending. If we only pull from pre-period mean, then we will have too high of a death rate to be actually comparable.
	...
	capture drop rate
	capture drop fe_age
	capture drop fe_county 
	capture drop fe_year
	
	// Generate rate
	if "`dem_type'" == "none" {
		gen rate = `death_type'/pop
	}
	// Generate rate
	if "`dem_type'" != "none" {
		gen rate = `death_type'/pop_`dem_type'
	}
	...
	*reghdfe rate if real_treatment == 0, absorb(fe_year_slope = c.year#i.unique_county_id fe_county = unique_county_id fe_year = year) savefe 
	reghdfe rate if real_ddd_treat == 0, absorb(fe_county = unique_county_id fe_year = year fe_age = age) savefe 
	...
	*bysort state county: carryforward fe_year_slope, replace
	...
	// Fill in FE for missing age
	sort state county year age
	sort  age year
	
	bysort age: carryforward fe_age, replace
	
	sort state county year age

	// Gen prediction 
	*gen double rate_hat = _b[_cons] + fe_county + fe_year + year*fe_year_slope
	gen double rate_hat = _b[_cons] + fe_county + fe_year + fe_age
	...
	*/
	...
	/*
	replace rate_hat = . if  age == "65_74"
	
	// Custom regression to estimate rate without post-treatment treated counties data
	*reghdfe rate if real_dd_treat == 0, absorb(fe_year_slope = c.year#i.unique_county_id fe_county = unique_county_id fe_year = year) savefe 
	capture drop fe_year_slope
	capture drop fe_county 
	capture drop fe_year

	reghdfe rate if real_dd_treat == 0 & age == "65_74", absorb(fe_county = unique_county_id fe_year = year) savefe 
	...
	*bysort state county: carryforward fe_year_slope, replace
	...
	// Fill in FE for missing county-year
	sort state county year
	bysort state county age: carryforward fe_county, replace
	
	// Fill in FE for missing year
	sort  year
	bysort year age: carryforward fe_year, replace
	*/
	// Gen prediction 
	*gen double rate_hat = _b[_cons] + fe_county + fe_year + year*fe_year_slope
	*replace rate_hat = _b[_cons] + fe_county + fe_year if age == "65_74" 
	*replace rate_hat = 0 if rate_hat < 0 & age == "65_74" // This did not come up in practice, but is possible in theory. Would render the binomial draw below useless. 
...
quietly {
		...
		*local first_year 2004
		capture drop rate_normal_55_64
		sum rate if year == `first_year' & age == "55_64"
		gen rate_normal_55_64 = rnormal(`r(mean)', `r(sd)') 
		replace rate_normal_55_64 = . if year != `first_year'
		replace rate_normal_55_64 = . if age == "65_74"
		replace rate_normal_55_64 = 0 if rate_normal_55_64 < 0 & !missing(rate_normal_55_64)
		
		
		capture drop rate_normal_65_74
		sum rate if year == `first_year' & age == "65_74"
		gen rate_normal_65_74 = rnormal(`r(mean)', `r(sd)') 
		replace rate_normal_65_74 = . if year != `first_year'
		replace rate_normal_65_74 = . if age == "55_64"
		replace rate_normal_65_74 = 0 if rate_normal_65_74 < 0 & !missing(rate_normal_65_74)
		
		
		capture drop rate_normal
		gen rate_normal = rate_normal_55_64
		replace rate_normal = rate_normal_65_74 if age == "65_74"
		
		capture drop new_rate_normal
		bysort state county age: egen new_rate_normal = max(rate_normal)
		
		sort  state county age year 

		capture drop order
		bysort state county age: gen order = _n
		
		*local last_year 2010
		reghdfe rate c.year if year <= `last_year' & age == "55_64", noabsorb 
		replace new_rate_normal = new_rate_normal + (order-1)*_b[year] if age == "55_64"
		replace new_rate_normal = 0 if new_rate_normal < 0 & age == "55_64"
		
		reghdfe rate c.year if year <= `last_year' & age == "65_74", noabsorb 
		replace new_rate_normal = new_rate_normal + (order-1)*_b[year] if age == "65_74"
		replace new_rate_normal = 0 if new_rate_normal < 0 & age == "65_74"
		
		sort state county  age year 
		order state county age year order rate_normal new_rate_normal	
		...
		if "`dem_type'" == "none" {		
			...
			gen synthetic_deaths = rbinomial(pop, rate_hat) 
		...
		*local x = 0
			...
			replace reduced_deaths_`counter' = rbinomial(synthetic_deaths,`x') if real_ddd_treat == 1
			...
			if "`prefix'" == "ln"{
				...
				qui reghdfe ln_dr_`counter' ///
					...
					`list_of_controls'  ///
					[aweight = `weight_var'], ///
					absorb(id year)  vce(cluster `cluster_var') 
				
				//Evaluate Effect using nlcom. Since we will do a tranform of the log results  
				 qui nlcom 100*(exp(_b[1.real_ddd_treat])-1)
