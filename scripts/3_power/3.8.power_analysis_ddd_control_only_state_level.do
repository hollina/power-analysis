...
capture program drop st_cntrl_only_power_analysis_ddd

// *----------
program define st_cntrl_only_power_analysis_ddd
syntax, ///
	end_value(real) ///
	step_size(real) ///
...
quietly {
	...
	*/
	...
	local file_name_suffix st_control_only_ddd_`prefix'_`death_type'_`dem_type'_cluster_`cluster_var'_weight_`weight_var'_controls_`list_of_controls'_preperiod_`first_year'_`last_year'
	...
	** Create Event Time Variable
	...
	compress
	save "$temp_path/st_ddd_temp_state_all.dta", replace
	
	use "$temp_path/st_ddd_temp_state_all.dta", clear
	...
	forvalues x = 1(1)`number_control' {
		...
		save "$temp_path/st_ddd_temp_state_id_`x'.dta", replace
	}	
...
quietly {	
		...
		*use "$temp_path/temp_data.dta", clear
		...
		forvalues x = 1(1)`number_states' {
			...
			di "`y'"
			append using "$temp_path/st_ddd_temp_state_id_`y'.dta"
	...
	order id new_state_id state  year
		...
		*drop if year >= 2014
		...
		foreach x in `step_macro' {
			...
			if "`dem_type'" != "none" {		
				gen reduced_deaths_`counter' = 0 
				replace reduced_deaths_`counter' = rbinomial(`death_type'_`dem_type',`x') if ddd_treat == 1
				replace reduced_deaths_`counter' = 0 if missing(reduced_deaths_`counter')
...
quietly {	
	...
	local number_control = number_control
	forvalues x = 1(1)`number_control' {
			erase "$temp_path/st_ddd_temp_state_id_`x'.dta"
	}	
	erase "$temp_path/st_ddd_temp_state_all.dta"
}
end	
